...
```
pip install sccore
manifest -m manifest.csv -f /workspaces/scatac_test_data/
```

`-m --manifest` Path to the manifest CSV file containing fastq_prefix-sample mapping.
...
```bash
nextflow run singleron-RD/scatac -profile docker -params-file params.yaml
```
...
```
pip install nf-core
nf-core launch singleron-RD/scatac
```
...
First, go to the [singleron-RD/scatac releases page](https://github.com/singleron-RD/scatac/releases) and find the latest pipeline version - numeric only (eg. `1.3.1`). Then specify this when running the pipeline with `-r` (one hyphen) - eg. `-r 1.3.1`. Of course, you can switch to another version by changing the number after the `-r` flag.
