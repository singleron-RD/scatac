...
def write_samplesheet(samplesheet_file, fastq_files, manifest):
    ...
    with open(samplesheet_file, 'w', newline='') as csvfile:
        fieldnames = ['sample', 'fastq_1', 'fastq_2', 'fastq_3']
        writer = csv.DictWriter(csvfile, fieldnames=fieldnames)
        writer.writeheader()
        for prefix, sample in manifest.items():
            r1,r2,r3 = fastq_files[sample][prefix][1], fastq_files[sample][prefix][2], fastq_files[sample][prefix][3]
            if len(r1) != len(r2):
            ...
            if not r1:
                ...
                continue
            for f1,f2,f3 in zip(sorted(r1),sorted(r2), sorted(r3)):
                writer.writerow({'sample': sample, 'fastq_1': f1, 'fastq_2': f2, 'fastq_3': f3})
            n += 1
    return n
